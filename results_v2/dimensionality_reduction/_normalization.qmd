...
dir <- "results_v2/dimensionality_reduction"
dir <- "."
...
metric_info <- jsonlite::read_json(paste0(dir, "/data/metric_info.json"), simplifyVector = TRUE) %>%
  rename(metric_id = id)
results <- jsonlite::read_json(paste0(dir, "/data/results.json"), simplifyVector = TRUE) %>%
...
overall_ranking <- results_long_scaled %>%
  group_by(method_id) %>%
  summarise(mean_score = mean(scaled_score, na.rm = TRUE)) %>%
