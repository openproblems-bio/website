...
task_info$task_description <- task_info$task_description %||% "missing 'task_description'"
...
method_info$method_summary <- method_info$method_summary %||% "missing 'method_summary'"

# read metric info
metric_info <- jsonlite::read_json(paste0(params$data_dir, "/metric_info.json"), simplifyVector = TRUE)
metric_info$metric_summary <- metric_info$metric_summary %||% "missing 'metric_summary'"
...
```{r}
#| echo: false
lines <- pmap_chr(method_info, function(method_name, method_summary, paper_reference, ...) {
  glue::glue("* **{method_name}** [@{paper_reference}]: {method_summary}")
})
...
#| echo: false
#| output: asis
pwalk(dataset_info, function(dataset_name, dataset_summary, data_reference, ...) {
  cat(glue::glue("* **{dataset_name}** [@{data_reference}]: {dataset_summary}"), "\n", sep = "")
})
```
...
#| echo: false
#| output: asis
pwalk(metric_info, function(metric_name, metric_summary, paper_reference, ...) {
  cat(glue::glue("* **{metric_name}** [@{paper_reference}]: {metric_summary}"), "\n", sep = "")
})
```
