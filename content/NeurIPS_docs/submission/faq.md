...
## Can I use a GPU to train the model

Absolutely! The evaluation worker runs on an Amazon EC2 G4dn instance, so it has 1 NVIDIA T4 GPU available for use. In order to make use of the GPU, there are three things to pay attention to:

* Make sure that the base Docker image contains the necessary Nvidia drivers and CUDA libraries. For instance, `pytorch/pytorch:1.9.0-cuda11.1-cudnn8-runtime`.
* It needs to be made visible to your component within a Docker container. Set the run arguments of the docker platform to `[ "--gpus all" ]` and add the `gpu` label to the nextflow platform.
* Add a `gpu` label to the `scripts/nextflow.config`.

The platforms section of your Viash config should look something like this:
```yaml
platforms:
  - type: docker
...
The next release of the starter kits will require less custom modifications in order to get GPU support to work.