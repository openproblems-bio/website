...
path <- paste0("results/", params$task_id, "/")
```
...
summary <- 
  method_info %>%
  transmute(method_id, method_name, method_is_baseline = ifelse(is_baseline, "yes", "")) %>%
...
column_info <- 
  tibble(
    ...
    name = id %>%
      ...
      str_to_title(),
    group = gsub("_.*", "", id),
    geom = c("method" = "text", "mean" = "bar", "dataset" = "funkyrect", "metric" = "funkyrect")[group],
    palette = c("method" = NA_character_, "mean" = "mean", "dataset" = "dataset", "metric" = "metric")[group],
