...
def main():
    ...
    for task_info_file in task_info_files:
        ...
        task_dest_path = task_info_file.parent

        print(f"Processing task {task_dest_path.parent.name}", flush=True)

        # read data from files
        with open(task_dest_path / "task_info.json", "r", encoding="utf8") as file:
            task_info = json.load(file)
        with open(task_dest_path / "method_info.json", "r", encoding="utf8") as file:
            method_info = json.load(file)
        with open(task_dest_path / "metric_info.json", "r", encoding="utf8") as file:
            metric_info = json.load(file)
        with open(task_dest_path / "dataset_info.json", "r", encoding="utf8") as file:
            dataset_info = json.load(file)
        with open(task_dest_path / "results.json", "r", encoding="utf8") as file:
