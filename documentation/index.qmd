...
data  = await supabase
  .from('timeline')
  .select('first_release, task, tasks ( task_name)' );
// flatten data array
...
// Add result links
UpdatedData = transformedData.map(item => ({
  ...item,
...

{
  for (let year = 2020; year <= 2025; year++) {
    timelineArray.push({ first_run: `${year}-01-01`, task: '', url: ''});
  }
}


combined=UpdatedData.concat(timelineArray);
``` -->