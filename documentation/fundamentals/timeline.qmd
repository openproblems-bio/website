...
function Swatches(color, {
  ...
  <div style=${{columns}}>${domain.map(value => {
    ...
    return htl.html`<div class=legend-swatches-item>
      <div class=legend-swatches-swatch style=${{background: color(value)}}></div>
      <div class=legend-swatches-label title=${label}>${label}</div>
    </div>`;
  })}
  </div>
</div>`;
  ...
  return htl.html`<div class=legend-swatches-item>
  ${domain.map(value => htl.html`<span class="${id}" style="--color: ${color(value)}">${format(value)}</span>`)}`;
}
```
