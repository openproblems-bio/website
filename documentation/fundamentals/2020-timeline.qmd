...
params = {
  ...
  output["svg"] = {
    "width":  width,
    "height": data.length * 100 // Roughly relative to number of data points but doesn't factor in the full timeline scale such as clustering or spread out data
  };
      ...
      <tbody>
        ${data.map(row => {
          return html`<tr>${Object.values(row).map((col, index) => {
            return index === 0 ? `<td>${d3.timeFormat("%Y")(col)}</td>`: `<td>${col}</td>`;
          })}</tr>`;
        })}
      </tbody>
    </table>
...
accessibleDataTable = render_data_table(data, {focusable: false})
```
...
DiDoesDigital2020Timeline = {
  ...
  svg.on("touchmove mousemove", function(event) {
    ...
    if (mouseY >= rangeY0 - fuzzyTextHeightAdjustment) {
      ...
      tooltip.select("#date")
        .text(d3.timeFormat("%Y")(dataEvent.date));
      tooltip.select("#name")
