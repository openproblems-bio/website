...
```{.yaml filename="src/process_dataset/config.vsh.yaml"}
...
6. A [Docker engine](https://viash.io/guide/component/add-dependencies.html) to make the component reproducible.
7. A [Nextflow runner](https://viash.io/guide/nextflow_vdsl3/create-a-module.html) to allow turning this component into a Nextflow module (and also standalone pipeline).
...
```{.python filename="src/process_dataset/script.py"}
...
```{.r filename="src/process_dataset/script.R"}
library(anndata)
...
```{.bash filename="scripts/create_test_resources.sh"}
#!/bin/bash
...
# # Run one metric
# viash run src/metrics/<metric_id>/config.vsh.yaml -- \
...
```bash
chmod +x scripts/create_test_resources.sh
scripts/create_test_resources.sh
```
