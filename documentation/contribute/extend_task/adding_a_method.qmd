...
You will find the control methods in the `control_methods` directory and the methods in the `methods` directory. The setup of both components is similar with small differences in the config yaml. 
...
To add a method create a new directory in the `src/task/methods` (where `task` is replaced by the task name. e.g. `denoising`) with the name of the new method in snake case. A task can also contain a subtask then this should be `src/task/subtask/methods`.
